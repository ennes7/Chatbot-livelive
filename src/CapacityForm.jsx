...
function AutoGrowTextarea(props){
  ...
  const onInput = () => { const el=ref.current; if(!el) return; el.style.height='0px'; el.style.height=Math.min(el.scrollHeight,600)+'px' }
  useEffect(()=>{ onInput() }, [])
  return <textarea ref={ref} className="textarea" onInput={onInput} {...props} />
}

function Field({ cfg, value, setValue }){
  const common = { value: value ?? '', onChange: e => setValue(e.target.value), placeholder: 'Optioneel' }
  return (
    <div className="field">
      <label>{cfg.label}</label>
      {cfg.type==='textarea' && <AutoGrowTextarea rows={6} {...common} />}
      {cfg.type==='text' && <input type="text" className="input" {...common} />}
      {cfg.type==='number' && <input type="number" className="input" min={cfg.min} max={cfg.max} {...common} />}
      {cfg.type==='date' && <input type="date" className="input" {...common} />}
      {cfg.type==='select' && (
      ...
      <div className="hint">Je kunt dit veld overslaan.</div>
    </div>
  )
}
